...
def generate_html():
    ...
    html_content = generate_html_template(all_years_data)
    ...
    for year, year_data in data.items():
        ...
        for activity in year_data['recent_activity']:
            ...
            for action in activity:
                activity_text += f'<p> {action["team"]} {action["player"]}</p>'
            trade_history += f'<div class="card">{activity_text}</div>'
        
        # Generate power rankings rows
        power_rankings_rows = ""
        for i, rank in enumerate(year_data['power_rankings']):
            power_rankings_rows += f"""
                    <tr>
                        <td>{i+1}</td>
                        <td>{rank['team']}</td>
                        <td>{rank['score']:.2f}</td>
                        <td>{rank['points']:.2f}</td>
                        <td>{rank['win_pct']*100:.1f}%</td>
                        <td>{rank['median_pct']*100:.1f}%</td>
                    </tr>"""
        
        # Generate statistics cards
        top_scorer_text = f'<p>{year_data["top_scorer"]["team"]} ({year_data["top_scorer"]["points"]} points)</p>' if year_data['top_scorer'] else '<p>No data available</p>'
        least_scorer_text = f'<p>{year_data["least_scorer"]["team"]} ({year_data["least_scorer"]["points"]} points)</p>' if year_data['least_scorer'] else '<p>No data available</p>'
        most_points_against_text = f'<p>{year_data["most_points_against"]["team"]} ({year_data["most_points_against"]["points"]} points)</p>' if year_data['most_points_against'] else '<p>No data available</p>'
        top_scored_week_text = f'<p>{year_data["top_scored_week"]["team"]} ({year_data["top_scored_week"]["score"]} points)</p>' if year_data['top_scored_week'] else '<p>No data available</p>'
        least_scored_week_text = f'<p>{year_data["least_scored_week"]["team"]} ({year_data["least_scored_week"]["score"]} points)</p>' if year_data['least_scored_week'] else '<p>No data available</p>'
            ...
            <div id="stats-{year}" class="tab-content card">
                ...
                <div class="grid">
                    ...
                    <div class="card">
                        ...
                        {most_points_against_text}
                    </div>
                    <div class="card">
                        <h4>Highest Scoring Week</h4>
                        {top_scored_week_text}
                    </div>
                    <div class="card">
                        <h4>Lowest Scoring Week</h4>
                        {least_scored_week_text}
                    </div>
                </div>
...
<body>
    ...
    <script>
        ...
        const mostRecentYear = Math.max(...Object.keys(data).map(Number));
